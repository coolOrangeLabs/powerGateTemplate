...
function Check-Boms($VaultBoms) {
	...
	foreach ($vaultBom in $VaultBoms) {
		...
		if (-not $number) {
			...
			Update-BomWindowEntity -InputObject $vaultBom -Status Error -StatusDetails "There is no erp material linked to this entity!"
			foreach ($vaultBomRow in $VaultBom.Children) {
				Update-BomWindowEntity -InputObject $vaultBomRow -Status Error -StatusDetails "BOM head has no linked erp material!"
			}
			continue
		}
		...
		if (-not $erpBomHeader) {
			...
			foreach ($vaultBomRow in $VaultBom.Children) {
				...
				if (-not $vaultBomRow.Bom_PositionNumber) {
					Update-BomWindowEntity -InputObject $vaultBomRow -Status Error -StatusDetails "Position property is empty."
					Update-BomWindowEntity -InputObject $vaultBom -Status Error -StatusDetails "BOM contains bomrows with a empty position property"
					continue
				}
				...
				if (-not $erpMaterial) {
					...
					Update-BomWindowEntity -InputObject $vaultBomRow -Status Error -StatusDetails "Position doesn't exist as Item in ERP"
					Update-BomWindowEntity -InputObject $vaultBom -Status Error -StatusDetails "BOM contains positions that do not exist as Items in ERP"
				}
				else {
					Update-BomWindowEntity -InputObject $vaultBomRow -Status New -StatusDetails "Position will be added to ERP"
				}
			}
			continue
		}
		...
		Update-BomWindowEntity -InputObject $vaultBom -Status Identical -StatusDetails "BOM is identical between Vault and ERP"
		foreach ($vaultBomRow in $VaultBom.Children) {
			$childNumber = GetEntityNumber -entity $vaultBomRow
			if (-not $vaultBomRow.Bom_PositionNumber) {
				Update-BomWindowEntity -InputObject $vaultBomRow -Status Error -StatusDetails "Position property is empty."
			...
			if ($erpBomRow) {
				if ($vaultBomRow.Bom_Quantity -eq $erpBomRow.Quantity) {
					Update-BomWindowEntity -InputObject $vaultBomRow -Status Identical -StatusDetails "Position is identical"
				}
				else {
					Update-BomWindowEntity -InputObject $vaultBomRow -Status Different -StatusDetails "Quantity is different: '$($vaultBomRow.Bom_Quantity) <> $($erpBomRow.Quantity)'"
					Update-BomWindowEntity -InputObject $vaultBom -Status Different -StatusDetails "BOMs are different between Vault and ERP!"
				}
		...
		foreach ($erpBomRow in $erpBomHeader.BomRows) {
			...
			if ($null -eq $vaultBomRow) {
				...
				Update-BomWindowEntity -InputObject $vaultBom -Status Different -StatusDetails "BOM rows are different between Vault and ERP!"
			}
		}
