...
$materialNumber = "VALVE"
$bomBug = "1209080702" #Nozzle.iam

$materials = Get-ERPObjects -EntitySet "Materials"
$material = Get-ERPObject -EntitySet "Materials" -Keys @{Number = $materialNumber }
...
$bom = Get-ERPObject -EntitySet "BomHeaders" -Keys @{Number = $bomBug } -Expand @('BomRows')
